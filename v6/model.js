...
Apigee.APIModel.Editor = function() {
  ...
  /*
   * Set editor mode - xml or json.
   * @param {string} mode A mimetype string  representing xml or json
   */
  this.setEditorMode = function(mode) {
    editor.setOption("mode", mode.endsWith("xml")?"xml":"javascript");
  };
};
...
Apigee.APIModel.Methods = function() {
  ...
  this.init = function() {
    ...
    // handle change events the content-type select.
    jQuery("[data-role='header-param-list']").each(function() {
      var $name = jQuery(this).find("[data-role='name']");
      var headerParamName = $name.text();
      if (headerParamName == "Content-Type") {
        var $selectElements = $name.parent().next().find('select[data-role="multiple-value"]');
        if ($selectElements.length == 1) {
          $selectElements.change(function(e){
            ...
            var $rp = jQuery('div.request_payload');
            var $textarea = $rp.find('[data-role="request-payload-example"] > textarea');
            $textarea.attr('data-format', value);
            window.apiModelEditor.setEditorMode(value); // CodeMirror does XML or JSON
            
            var sampleFromAPISchema;
            ...
            if (value.endsWith('xml')) {
              // TODO: select a name for the root element
              sampleFromAPISchema = swaggerModel.createXMLSample( false, false );
              jQuery("textarea.payload_text").val(sampleFromAPISchema); // necessary?
              window.apiModelEditor.setRequestPayLoad(sampleFromAPISchema);
            }
            else {
              sampleFromAPISchema = JSON.stringify(swaggerModel.createJSONSample( false ), null, 4);
              jQuery("textarea.payload_text").val(sampleFromAPISchema); // necessary?
              window.apiModelEditor.setRequestPayLoad(sampleFromAPISchema);
            }
          });
...
Apigee.APIModel.SwaggerModel = function(modelName, obj) {
  ...
  this.createJSONSample = function(modelsToIgnore) {
    var cacheKey = 'json-' + this.name;
    if(Apigee.APIModel.sampleModels[cacheKey]) {
      return Apigee.APIModel.sampleModels[cacheKey];
    }
    else {
      var result = {};
      ...
      Apigee.APIModel.sampleModels[cacheKey] = result;
      modelsToIgnore.pop();
      return result;
    }
  };
  ...
  function getPrefix(putativePrefix) {
    return putativePrefix || ('ns' + xmlNsPrefixCounter++);
  }
  
  this.createXMLSample = function(elementName, modelsToIgnore) {
    var cacheKey = 'xml-' + this.name;
    elementName = elementName || this.name
    if(Apigee.APIModel.sampleModels[cacheKey]) {
      return Apigee.APIModel.sampleModels[cacheKey];
    }
    else {
      var xmlDoc = (this.xml && this.xml.namepace) ? 
        window.document.implementation.createDocument( this.xml.namepace, getPrefix(this.xml.prefix) + ':' + elementName) :
        window.document.implementation.createDocument( null, elementName);
      modelsToIgnore = (modelsToIgnore||[]);
      if(modelsToIgnore.indexOf(this.name)!== -1){
        return this.name;
      }
      ...
      for (var i = 0; i < this.properties.length; i++) {
        prop = this.properties[i];
        prop.appendSampleXMLNode(xmlDoc, modelsToIgnore);
      }
      var sXML = prettyPrintXml((new XMLSerializer()).serializeToString(xmlDoc));
      Apigee.APIModel.sampleModels[cacheKey] = sXML;
      modelsToIgnore.pop();
      return sXML;
    }
  };
...
Apigee.APIModel.SwaggerModelProperty = function(name, obj) {
  ...
  this.appendSampleXMLNode = function(xmlDoc, modelsToIgnore) {
    modelsToIgnore = (modelsToIgnore||[]);
    var resultNode;
    if ((this.refModel != null) && (modelsToIgnore.indexOf(this.refModel.name) === -1)) {
      var xmlText = this.refModel.createXMLSample(this.name, modelsToIgnore);
      var newdoc = jQuery.parseXML( xmlText );
      resultNode = xmlDoc.importNode(newdoc.documentElement, true);
    }
    else {
      if (this.isCollection) {
        resultNode = this.toSampleXMLNode(xmlDoc, this.refDataType, modelsToIgnore);
      }
      else {
        resultNode = this.toSampleXMLNode(xmlDoc, this.dataType, modelsToIgnore);
      }
    }
    var root = xmlDoc.documentElement;
    if (this.isCollection) {
      // not sure this is valid for attributes, so assume elements
      root.appendChild(resultNode);
      root.appendChild(resultNode.cloneNode(true));
      return [resultNode];
    }
    else {
      if (resultNode.__proto__.constructor.name == 'Attr') {
        root.setAttribute(resultNode.name, resultNode.value);
      }
      else {
        root.appendChild(resultNode);
      }
  ...
  this.toSampleXMLNode = function(xmlDoc, value, modelsToIgnore) {
    var resultNode;
    if (this.xml && this.xml.attribute) {
      // this does not handle attributes that are namespace-qualified (XML is stupid complex!)
      resultNode = xmlDoc.createAttribute(this.name)
      resultNode.value = this.toSampleValue(value, modelsToIgnore);
    }
    else {
      // element
      var xmlns = (this.xml && this.xml.namespace) ? this.xml.namespace: false;
      var xmlnsPrefix = (this.xml && this.xml.prefix) ? this.xml.prefix: "ns1";
      var open = '<' + this.name;
      if (xmlns) {
        open += ' xmlns:' + xmlnsPrefix + '="' + xmlns + '"';
      }
      open += '>';
      var close = '</' + this.name + '>';
      ...
      var xmlText;
      if (value === "integer") {
        xmlText = open + (counter++) + close;
      } else if (value === "boolean") {
        xmlText = open + 'false' + close;
      } else if (value === "double" || value === "number") {
        xmlText = open + '0.0' + close;
      } else if (value === "string") {
        xmlText = open + 'Abcd' + close;
      } else if (Apigee.APIModel.originalExpandedSchema[value]) {
        var swaggerModel = new Apigee.APIModel.SwaggerModel( value, Apigee.APIModel.originalExpandedSchema[value]);
        xmlText = swaggerModel.createXMLSample(this.name, modelsToIgnore);
      } else {
        xmlText = open + value + close;
      }
      var newdoc = jQuery.parseXML( xmlText );
      resultNode = xmlDoc.importNode(newdoc.documentElement, true);
    }
    // Do not actually append the node. Just return it.
    return resultNode;
  };
};
